	...
	async function getCloudPrefix() {
        ...
        window.onStudyModeChange = function() {
            ...
            if (selectedMode === 'doomscroll') {
                if (catContainer) catContainer.classList.add('hidden');
                if (setProgressContainer) setProgressContainer.classList.add('hidden');
                if (launchBtn) {
                    launchBtn.disabled = false;
                    launchBtn.innerText = "Begin Doomscroll";
                }
            } else {
        ...
        function startStandardSession() {
            ...
            if (selectedStudyMode === 'doomscroll') {
                if (typeof startDoomscrollSession === 'function') {
        ...
        function launchCoreSession() {
            ...
            if (!isReviewMode) {
                const progressKey = mode === 'doomscroll' ? 'doomscroll_progress' : `${catSelect.value}|${setSelect.value}`;
                currentIndex = parseInt(progressDB[progressKey]) || 0;
                
                if (typeof sessionMatchedIndices !== 'undefined' && mode !== 'doomscroll') {
        ...
        function endSession() {
            ...
            if (mode === 'doomscroll' && typeof stopAllDoomscrollVideos === 'function') {
                stopAllDoomscrollVideos();
            }
            if(isSessionComplete || confirm("Exit the current study session? Progress is saved locally.")) {
                ...
                const dsFeed = document.getElementById('doomscroll-feed');
                if (dsFeed && dsFeed._keyHandler) { document.removeEventListener('keydown', dsFeed._keyHandler); dsFeed._keyHandler = null; }
                updateDashboard();
                document.getElementById('matching-container').classList.add('hidden');
                document.getElementById('doomscroll-container').classList.add('hidden');
                toggleCategory(true);
                showSetup();
                pushToCloud();
            }
        ...
        function updateSessionProgress() {
            ...
            if (mode === 'matching') {
                ...
                const wordNum = Math.min(currentIndex + 1, appData.length);
                progressText.innerText = `Word ${wordNum} of ${appData.length}`;
                const pct = appData.length > 0 ? (wordNum / appData.length) * 100 : 0;
                progressBar.style.width = `${pct}%`;
        ...
        function renderCard() {
            if (mode === 'doomscroll') {
                ...
                document.getElementById('doomscroll-container').classList.remove('hidden');
                document.getElementById('doomscroll-container').classList.add('flex');
                document.getElementById('next-btn-container').classList.add('hidden');
                if (typeof renderDoomscrollFeed === 'function') {
                    renderDoomscrollFeed();
                }
                return;
            }
            isFlipped = false;
        ...
        // =========================================================
        // DOOMSCROLL MODE IMPLEMENTATION
        // =========================================================
        let doomscrollScrollTimeout = null;
        let doomscrollAutoScrollTimeout = null;
        let doomscrollCloseTimeout = null;
        let lastActiveIndex = -1;
        const doomscrollTempLikes = {};

        async function startDoomscrollSession() {
            try {
                // 1. Get vocab groups from globally loaded database.js
                const vocabGroups = typeof VOCAB_GROUPS !== 'undefined' ? VOCAB_GROUPS : [];
                
                // 2. Build unique word lookup for GRE-Essential
                const essentialWordsMap = new Map();
                if (vocabularyDB["GRE-Essential"]) {
                    Object.keys(vocabularyDB["GRE-Essential"]).forEach(setName => {
                        vocabularyDB["GRE-Essential"][setName].forEach(wordObj => {
                            essentialWordsMap.set(wordObj.word.toLowerCase().trim(), wordObj);
                        });
                    });
                }
                ...
                // 3. First pass: sort words in VOCAB_GROUPS order (related words one after another)
                vocabGroups.forEach(group => {
                    group.words.forEach(w => {
                        const cleanW = w.toLowerCase().trim();
                        if (essentialWordsMap.has(cleanW) && !seenWords.has(cleanW)) {
                            seenWords.add(cleanW);
                            const copy = { ...essentialWordsMap.get(cleanW) };
                            copy.groupName = "Group: " + group.name;
                            sortedWords.push(copy);
                        }
                    });
                });
                
                // 4. Second pass: collect remaining GRE-Essential words not present in any group
                const remainingWords = [];
                essentialWordsMap.forEach((wordObj, cleanW) => {
                    if (!seenWords.has(cleanW)) {
                        seenWords.add(cleanW);
                        const copy = { ...wordObj };
                        copy.groupName = "Ungrouped Essential Words";
                        remainingWords.push(copy);
                    }
                });
                // Sort remaining words alphabetically to keep them tidy
                remainingWords.sort((a, b) => a.word.localeCompare(b.word));
                
                // Combine them to get exactly all 644 unique words
                const combinedWords = sortedWords.concat(remainingWords);
                
                // Lowercase all words as requested
                combinedWords.forEach(w => {
                    w.word = w.word.toLowerCase();
                });
                
                // Separate words with video and completely hide words with no video
                const hasVideo = (w) => typeof VIDEO_WORDS_SET !== 'undefined' && VIDEO_WORDS_SET.has(w.word.trim().toLowerCase());
                
                const withVideo = combinedWords.filter(w => hasVideo(w));
                
                // Sort "calumny", "perfidious", "renege", "moribund" to the top in that exact order
                const priority = ["calumny", "perfidious", "renege", "moribund"];
                const priorityWords = [];
                const normalWords = [];
                
                withVideo.forEach(w => {
                    const cleanW = w.word.trim().toLowerCase();
                    if (priority.includes(cleanW)) {
                ...
                priorityWords.sort((a, b) => {
                    return priority.indexOf(a.word.trim().toLowerCase()) - priority.indexOf(b.word.trim().toLowerCase());
                });

                // Sort normalWords according to VIDEO_WORDS_SET order so new videos appear at the top
                const videoWordsOrder = typeof VIDEO_WORDS_SET !== 'undefined' ? Array.from(VIDEO_WORDS_SET) : [];
                normalWords.sort((a, b) => {
                    const idxA = videoWordsOrder.indexOf(a.word.trim().toLowerCase());
                    const idxB = videoWordsOrder.indexOf(b.word.trim().toLowerCase());
                ...
                if (appData.length === 0) {
                    showToast("No words found for Doomscroll mode", "error");
                    return;
                }
                ...
                document.getElementById('session-title').innerText = "GRE-ESSENTIAL";
                
                // Cancel any pending auto-end timeout from a previous session
                if (sessionEndTimeout) { clearTimeout(sessionEndTimeout); sessionEndTimeout = null; }
                
                // Load saved index
                const progressKey = 'doomscroll_progress';
                currentIndex = parseInt(progressDB[progressKey]) || 0;
                if (currentIndex >= appData.length) {
                    currentIndex = 0;
                }
                
                launchCoreSession();
                makeCommentsDrawerDraggable();
            } catch (e) {
                console.error("Error launching doomscroll session:", e);
                showToast("Failed to load Doomscroll data: " + e.message, "error");
            }
        }

        function renderDoomscrollFeed() {
            const feed = document.getElementById('doomscroll-feed');
            feed.innerHTML = '';
            
            appData.forEach((wordObj, i) => {
                const card = document.createElement('div');
                card.className = 'doomscroll-card';
                card.dataset.index = i;
                card.dataset.word = wordObj.word;
                
                // State indicator isLearned (Tick Mark)
                const isLearned = isDoomscrollWordLearned(wordObj.word);
                const learnedBtnClass = 'p-2 bg-transparent text-white drop-shadow-[0_2.5px_4px_rgba(0,0,0,0.9)] border-none focus:outline-none focus:ring-0 select-none';
                
                const learnedSvg = isLearned 
                    ? `<svg class="w-6 h-6 pointer-events-none" viewBox="0 0 24 24">
                           <circle cx="12" cy="12" r="10" fill="white" stroke="white" stroke-width="1.6"></circle>
                           <path stroke="black" stroke-width="2.0" stroke-linecap="round" stroke-linejoin="round" fill="none" d="M9 12l2 2 4-4"></path>
                       </svg>`
                    : `<svg class="w-6 h-6 pointer-events-none" viewBox="0 0 24 24">
                           <circle cx="12" cy="12" r="10" fill="none" stroke="currentColor" stroke-width="1.6"></circle>
                           <path stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none" d="M9 12l2 2 4-4"></path>
                       </svg>`;
                
                // State indicator isBookmarked (Bookmark list)
                const isBookmarked = isDoomscrollWordBookmarked(wordObj.word);
                const bookmarkBtnClass = 'p-2 bg-transparent text-white drop-shadow-[0_2.5px_4px_rgba(0,0,0,0.9)] border-none focus:outline-none focus:ring-0 select-none';
                const bookmarkSvgFill = isBookmarked ? '#ffffff' : 'none';
                const bookmarkSvgStroke = isBookmarked ? '#ffffff' : 'currentColor';
                
                // We render both the video tag and the fallback audio card. 
                // If the video fails to load (error event fired), it automatically swaps to fallback.
                card.innerHTML = `
                    <div class="w-full h-full relative flex items-center justify-center bg-black">
                        <!-- Video element. preload="none" ensures we don't load 644 videos simultaneously -->
                        <video class="w-full h-full object-contain doomscroll-video" src="videos/${wordObj.word}.mp4" preload="none" playsinline></video>
                        
                        <!-- Fallback visual audio card representation (hidden by default) -->
                        <div class="absolute inset-0 hidden fallback-audio-card bg-gradient-to-br from-teal-600 via-emerald-600 to-indigo-700 dark:from-neutral-900 dark:via-neutral-800 dark:to-neutral-950 flex flex-col items-center justify-center p-8 text-center select-none">
                            <div class="absolute inset-0 opacity-[0.03] dark:opacity-[0.06] pointer-events-none" style="background-image: radial-gradient(circle, #fff 10%, transparent 11%); background-size: 20px 20px;"></div>
                            <!-- Audio Wave Animation -->
                            <div class="flex gap-1 items-end h-12 mb-6 audio-wave">
                                <span class="w-1.5 bg-white/60 dark:bg-teal-500 rounded-full transition-all duration-300 wave-bar wave-bar-1"></span>
                                <span class="w-1.5 bg-white/60 dark:bg-teal-500 rounded-full transition-all duration-300 wave-bar wave-bar-2"></span>
                                <span class="w-1.5 bg-white/60 dark:bg-teal-500 rounded-full transition-all duration-300 wave-bar wave-bar-3"></span>
                                <span class="w-1.5 bg-white/60 dark:bg-teal-500 rounded-full transition-all duration-300 wave-bar wave-bar-4"></span>
                                <span class="w-1.5 bg-white/60 dark:bg-teal-500 rounded-full transition-all duration-300 wave-bar wave-bar-5"></span>
                            </div>
                            <h2 class="text-4xl sm:text-5xl font-serif font-black text-white tracking-wide mb-2 drop-shadow-md lowercase">${wordObj.word}</h2>
                            <span class="px-2.5 py-0.5 bg-white/20 backdrop-blur-sm border border-white/25 rounded-full text-[10px] text-white/90 uppercase tracking-widest font-semibold italic">${wordObj.type}</span>
                        </div>
                        ...
                        <div class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 transition-opacity duration-200 play-pause-overlay">
                            <div class="p-4 bg-black/50 rounded-full text-white">
                                <svg class="w-10 h-10 play-icon hidden" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd"></path></svg>
                                <svg class="w-10 h-10 pause-icon hidden" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zM7 8a1 1 0 012 0v4a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v4a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Vertical Action Tray (Reels-style, transparent background buttons) -->
                    <div class="absolute right-2 bottom-48 flex flex-col gap-4 items-center z-10 pointer-events-auto">
                        <!-- Comments -->
                        <div class="flex flex-col items-center">
                            <button onclick="toggleDoomscrollComments(event, ${i})" class="p-2 bg-transparent text-white drop-shadow-[0_2.5px_4px_rgba(0,0,0,0.9)] border-none focus:outline-none focus:ring-0 select-none" title="Comments">
                            ...
                            <span class="text-[10px] font-medium text-white/80 drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)] leading-none mt-[-1px] select-none" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">3</span>
                        </div>
                        
                        <!-- Bookmark -->
                        <button onclick="toggleDoomscrollBookmark(event, '${wordObj.word}', ${i})" id="doomscroll-bookmark-${i}" class="${bookmarkBtnClass}" title="Bookmark Word">
                            <svg class="w-6 h-6 pointer-events-none" fill="${bookmarkSvgFill}" stroke="${bookmarkSvgStroke}" stroke-width="1.6" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path>
                            </svg>
                        </button>

                        <!-- Tick (Mark Learned) -->
                        <button onclick="toggleDoomscrollLearned(event, '${wordObj.word}', ${i})" id="doomscroll-learned-${i}" class="${learnedBtnClass}" title="Mark Learned">
                            ${learnedSvg}
                        </button>
                    </div>
                `;
                
                const video = card.querySelector('.doomscroll-video');
                const fallback = card.querySelector('.fallback-audio-card');
                
                // Error listener on video: if file does not exist, hide video and show fallback immediately.
                video.addEventListener('error', () => {
                    video.classList.add('hidden');
                    fallback.classList.remove('hidden');
                    video.dataset.hasVideo = 'false';
                    
                    // If this is the active card, immediately initiate voice synthesis
                    if (i === currentIndex && !('speechSynthesis' in window && window.speechSynthesis.speaking)) {
                        speakLongExample(wordObj, card);
                    }
                });
                ...
                // Add click handler on media container to toggle play/pause
                const mediaContainer = card.querySelector('.bg-black') || fallback;
                if (mediaContainer) {
                    mediaContainer.style.cursor = 'pointer';
                    mediaContainer.addEventListener('click', (e) => {
                        if (e.target.closest('button')) return;
                        
                        // If comments are open, close them and do NOT toggle media playback
                        const drawer = document.getElementById('doomscroll-comments-drawer');
                        if (drawer && !drawer.classList.contains('translate-y-full')) {
                            window.closeDoomscrollComments();
                            return;
                        }
                        
                        togglePlayPauseActiveMedia();
                    });
                }
            ...
            feed.removeEventListener('scroll', handleDoomscrollScroll);
            feed.addEventListener('scroll', handleDoomscrollScroll);
            
            // Prevent skipping multiple reels with wheel/trackpad scrolling on desktop
            let isDoomscrollScrolling = false;
            feed.onwheel = (e) => {
                // If comments drawer is open, block all feed wheel scrolling completely
                const drawer = document.getElementById('doomscroll-comments-drawer');
                if (drawer && !drawer.classList.contains('translate-y-full')) {
                    e.preventDefault();
                    return;
                ...
                e.preventDefault();
                if (isDoomscrollScrolling) return;
                
                const direction = e.deltaY > 0 ? 1 : -1;
                const nextIdx = currentIndex + direction;
                
                if (nextIdx >= 0 && nextIdx < appData.length) {
                    isDoomscrollScrolling = true;
                    const nextCard = feed.querySelector(`.doomscroll-card[data-index="${nextIdx}"]`);
                    if (nextCard) {
                        nextCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        setTimeout(() => {
                            isDoomscrollScrolling = false;
                        }, 650); // lock scrolling input for 650ms during animation
                    } else {
                        isDoomscrollScrolling = false;
                    }
                }
            ...
            const touchMoveHandler = (e) => {
                // If comments drawer is open, do not block native scroll inside the drawer list
                const drawer = document.getElementById('doomscroll-comments-drawer');
                if (drawer && !drawer.classList.contains('translate-y-full')) {
                    return; 
                }
                ...
                // If swipe distance exceeds 50px, snap exactly 1 index
            ...
            if (feed._touchStartHandler) {
                feed.removeEventListener('touchstart', feed._touchStartHandler);
            }
            if (feed._touchMoveHandler) {
                feed.removeEventListener('touchmove', feed._touchMoveHandler, { passive: false });
            }

            feed._touchStartHandler = touchStartHandler;
            feed._touchMoveHandler = touchMoveHandler;

            feed.addEventListener('touchstart', touchStartHandler, { passive: true });
            feed.addEventListener('touchmove', touchMoveHandler, { passive: false });

            // Lock page scroll so comments drawer can't be seen behind
            const mainEl = document.querySelector('main');
            if (mainEl) {
            ...
            // Prevent spacebar from scrolling on PC
            if (feed._keyHandler) document.removeEventListener('keydown', feed._keyHandler);
            feed._keyHandler = (e) => {
                if (e.code === 'Space' || e.key === ' ') {
                    e.preventDefault();
                }
            };
            document.addEventListener('keydown', feed._keyHandler);
            
            // Wait for DOM layout, then scroll to the saved index
            setTimeout(() => {
                const activeCard = feed.querySelector(`.doomscroll-card[data-index="${currentIndex}"]`);
                if (activeCard) {
                    activeCard.scrollIntoView({ block: 'start' });
                    activateDoomscrollWord(currentIndex);
                } else {
                    activateDoomscrollWord(0);
                }
            }, 100);
        }

        function handleDoomscrollScroll() {
            const feed = document.getElementById('doomscroll-feed');
            if (!feed) return;
            ...
            const activeIndex = Math.round(feed.scrollTop / cardHeight);
            
            if (activeIndex !== lastActiveIndex && activeIndex >= 0 && activeIndex < appData.length) {
                currentIndex = activeIndex;
                updateSessionProgress();
                
                // Immediately start loading active video to decrease latency (before debounce fires)
                const activeCard = document.querySelector(`.doomscroll-card[data-index="${activeIndex}"]`);
                if (activeCard) {
                    const activeVideo = activeCard.querySelector('.doomscroll-video');
                    if (activeVideo && activeVideo.dataset.hasVideo !== 'false' && activeVideo.preload !== 'auto') {
                        activeVideo.preload = 'auto';
                        activeVideo.load();
                    }
                }
                
                if (doomscrollScrollTimeout) {
                    clearTimeout(doomscrollScrollTimeout);
                }
                doomscrollScrollTimeout = setTimeout(() => {
                    activateDoomscrollWord(activeIndex);
                }, 200);
            }
        }

        function activateDoomscrollWord(index) {
            const prevIndex = currentIndex;
            if (index !== currentIndex) {
                for (const k in doomscrollTempLikes) delete doomscrollTempLikes[k];
            }
            lastActiveIndex = index;
            currentIndex = index;
            
            // Don't close the comments drawer on accidental scroll nudges for the same word
            const drawer = document.getElementById('doomscroll-comments-drawer');
            const drawerOpen = drawer && !drawer.classList.contains('translate-y-full');
            if (!drawerOpen || index !== prevIndex) {
                closeDoomscrollComments();
            }
            
            if (doomscrollAutoScrollTimeout) {
                clearTimeout(doomscrollAutoScrollTimeout);
                doomscrollAutoScrollTimeout = null;
            }
            ...
            // Stop and reset all other videos
            const videos = document.querySelectorAll('.doomscroll-video');
            videos.forEach(v => {
                try {
                    v.pause();
                    v.currentTime = 0;
            ...
            // Reset all wave bars animations
            ...
            const activeCard = document.querySelector(`.doomscroll-card[data-index="${index}"]`);
            if (!activeCard) return;
            
            const wordObj = appData[index];
            const video = activeCard.querySelector('.doomscroll-video');
            
            if (video && video.dataset.hasVideo !== 'false') {
                video.muted = false;
                video.loop = true;
                
                // Only load if not already set to auto preload, avoiding black flash
                ...
                if (playPromise !== undefined) {
                    playPromise.catch(error => {
                        console.log("Video auto-play blocked or failed:", error);
                    });
                }
            } else {
                speakLongExample(wordObj, activeCard);
            }
            
            // Preload the next 2 videos ahead of time (offscreen loading)
            for (let offset = 1; offset <= 2; offset++) {
                const nextCard = document.querySelector(`.doomscroll-card[data-index="${index + offset}"]`);
                if (nextCard) {
                    const nextVideo = nextCard.querySelector('.doomscroll-video');
                    if (nextVideo && nextVideo.dataset.hasVideo !== 'false' && nextVideo.preload !== 'auto') {
        ...
        function speakLongExample(wordObj, card) {
            if ('speechSynthesis' in window) {
                window.speechSynthesis.cancel();
            }
            ...
            if (sentence) {
                ...
                const activeWaveBars = card.querySelectorAll('.wave-bar');
                activeWaveBars.forEach(bar => {
                    bar.style.height = `${16 + Math.random() * 24}px`;
                ...
                utterance.onend = () => {
                    ...
                    doomscrollAutoScrollTimeout = setTimeout(() => {
                        const cardIndex = parseInt(card.dataset.index);
                        if (currentIndex === cardIndex) {
                            speakLongExample(wordObj, card);
                        }
                    }, 1500); // 1.5s delay before looping definition voice
                };
        ...
        function togglePlayPauseActiveMedia() {
            const activeCard = document.querySelector(`.doomscroll-card[data-index="${currentIndex}"]`);
            if (!activeCard) return;
            ...
            if (video && video.dataset.hasVideo !== 'false') {
                ...
                // Speech synthesis pause/resume (tap to pause, not startover)
                if ('speechSynthesis' in window) {
                    if (window.speechSynthesis.speaking) {
                        if (window.speechSynthesis.paused) {
                            window.speechSynthesis.resume();
                            toggleWaveBarsAnimation(activeCard, true);
                        } else {
                            window.speechSynthesis.pause();
                            toggleWaveBarsAnimation(activeCard, false);
                        }
                    } else {
                        // Replay from start if not speaking
                        speakLongExample(wordObj, activeCard);
                    }
                }
            }
        ...
        function toggleWaveBarsAnimation(card, run) {
            const activeWaveBars = card.querySelectorAll('.wave-bar');
            activeWaveBars.forEach(bar => {
                if (run) {
                    bar.classList.add('animate-pulse');
        ...
        window.isDoomscrollWordLearned = function(word) {
            const learnedList = safeLocalStorageGetJSON('learned-words', []);
            const learnedSet = new Set(learnedList.map(w => w.trim().toLowerCase()));
            return learnedSet.has(word.trim().toLowerCase());
        };
        
        window.toggleDoomscrollLearned = function(event, word, index) {
            ...
            const btn = document.getElementById(`doomscroll-learned-${index}`);
            ...
            if (learnedSet.has(cleanWord)) {
                ...
                showToast(`Marked "${word}" as not learned`, "info");
            } else {
                learnedSet.add(cleanWord);
                if (btn) {
                    btn.innerHTML = `
                ...
                showToast(`Marked "${word}" as learned`, "success");
            }
        ...
        window.isDoomscrollWordBookmarked = function(word) {
            const bookmarksList = safeLocalStorageGetJSON('doomscroll-bookmarks', []);
            const bookmarksSet = new Set(bookmarksList.map(w => w.trim().toLowerCase()));
            return bookmarksSet.has(word.trim().toLowerCase());
        };

        window.toggleDoomscrollBookmark = function(event, word, index) {
            if (event) event.stopPropagation();
            haptic();
            ...
            const btn = document.getElementById(`doomscroll-bookmark-${index}`);
            
            if (bookmarksSet.has(cleanWord)) {
                bookmarksSet.delete(cleanWord);
                if (btn) {
                    btn.className = 'p-2 bg-transparent text-white drop-shadow-[0_2.5px_4px_rgba(0,0,0,0.9)] border-none focus:outline-none focus:ring-0 select-none';
                    const svg = btn.querySelector('svg');
                    if (svg) {
                ...
                showToast(`Bookmarked "${word}"`, "success");
            }
            
            localStorage.setItem('doomscroll-bookmarks', JSON.stringify(Array.from(bookmarksSet)));
        };

        window.toggleDoomscrollComments = function(event, index) {
            if (event) event.stopPropagation();
            haptic();
            ...
            if (drawer.classList.contains('translate-y-full')) {
                ...
                const rand1 = seededRandom(wordObj.word + '_gre');
                const rand2 = seededRandom(wordObj.word + '_ex');
                const rand3 = seededRandom(wordObj.word + '_long');
                
                // 1. Definition Comment (Creator Caption style)
                if (wordObj.def) {
                    const greLikes = Math.floor(rand1() * 16) + 10;
                    const greKey = wordObj.word + '_gre';
                    ...
                    <div class="flex items-start justify-between gap-3 text-xs">
                        <div class="flex items-start gap-3 w-full">
                            ...
                            <div class="flex flex-col flex-grow">
                                <div class="text-slate-800 dark:text-neutral-200">
                                    <span class="font-bold mr-1 text-slate-900 dark:text-white doomscroll-wordsource">@gre_essential</span>
                                    ...
                                    <div class="my-2.5 p-4 rounded-xl border border-slate-200/60 dark:border-neutral-800 bg-slate-50 dark:bg-neutral-900/60 flex flex-col items-center justify-center text-center shadow-sm select-none">
                                        <span class="text-xl font-serif font-black text-slate-800 dark:text-white lowercase tracking-wide doomscroll-wordname">${wordObj.word}</span>
                                        <span class="text-[10px] text-slate-450 dark:text-neutral-500 italic mt-0.5 lowercase doomscroll-wordtype">${wordObj.type}</span>
                                    </div>
                                    
                                    <p class="leading-relaxed mt-2 text-slate-700 dark:text-neutral-300 doomscroll-worddef">${wordObj.def}</p>
                                </div>
                                <!-- Footer Actions -->
                        ...
                        <div class="flex flex-col items-center shrink-0 mt-1 select-none">
                            <button class="${greBtnClass}" onclick="toggleDoomscrollCommentLike(this)" data-like-key="${greKey}">
                                <svg class="w-3.5 h-3.5" fill="${greFill}" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                </svg>
                            </button>
                            <span class="text-[9px] text-slate-400 dark:text-neutral-500 mt-0.5 like-count" data-base="${greLikes}">${greDisplay}</span>
                        </div>
                    </div>`;
                }
                ...
                if (wordObj.example) {
                    let regex = new RegExp("(" + escapeRegExp(wordObj.word) + "[a-zA-Z]*)", "gi");
                    let highlightedEx = wordObj.example.replace(regex, (match) => `<strong class="font-bold text-primary-600 dark:text-primary-400">${match}</strong>`);
                    ...
                    <div class="flex items-start justify-between gap-3 text-xs">
                        <div class="flex items-start gap-3 w-full">
                            ...
                            <div class="flex flex-col flex-grow">
                                <div class="text-slate-800 dark:text-neutral-200 flex items-center gap-1.5 flex-wrap">
                                    <span class="font-bold text-slate-900 dark:text-white doomscroll-wordsource">@example_sentence</span>
                                    ...
                                    <span class="text-[10px] text-slate-400 dark:text-neutral-500 doomscroll-wordsource">by author</span>
                                </div>
                        ...
                        <div class="flex flex-col items-center shrink-0 mt-1 select-none">
                            <button class="${exBtnClass}" onclick="toggleDoomscrollCommentLike(this)" data-like-key="${exKey}">
                                <svg class="w-3.5 h-3.5" fill="${exFill}" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                </svg>
                            </button>
                            <span class="text-[9px] text-slate-400 dark:text-neutral-500 mt-0.5 like-count" data-base="${exLikes}">${exDisplay}</span>
                        </div>
                    </div>`;
                }
                ...
                if (wordObj.long_example) {
                    let regex = new RegExp("(" + escapeRegExp(wordObj.word) + "[a-zA-Z]*)", "gi");
                    let highlightedLong = wordObj.long_example.replace(regex, (match) => `<strong class="font-bold text-primary-600 dark:text-primary-400">${match}</strong>`);
                    ...
                    <div class="flex items-start justify-between gap-3 text-xs">
                        <div class="flex items-start gap-3 w-full">
                            ...
                            <div class="flex flex-col flex-grow">
                                <div class="text-slate-800 dark:text-neutral-200 flex items-center gap-1.5 flex-wrap">
                                    <span class="font-bold text-slate-900 dark:text-white doomscroll-wordsource">@long_example</span>
                                    <svg class="w-2.5 h-2.5 text-red-500 fill-current inline-block" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                                    <span class="text-[10px] text-slate-400 dark:text-neutral-500 doomscroll-wordsource">by author</span>
                                </div>
                        ...
                        <div class="flex flex-col items-center shrink-0 mt-1 select-none">
                            <button class="${longBtnClass}" onclick="toggleDoomscrollCommentLike(this)" data-like-key="${longKey}">
                                <svg class="w-3.5 h-3.5" fill="${longFill}" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path>
                                </svg>
                            </button>
                            <span class="text-[9px] text-slate-400 dark:text-neutral-500 mt-0.5 like-count" data-base="${longLikes}">${longDisplay}</span>
                        </div>
                    </div>`;
                }
                
                document.getElementById('doomscroll-comments-list').innerHTML = commentsHtml || '<p class="text-sm text-slate-400 dark:text-slate-500 text-center py-6">No context available.</p>';

                // Cancel any pending scroll-triggered activate so it doesn't close the drawer we're about to open
                if (doomscrollScrollTimeout) {
                    clearTimeout(doomscrollScrollTimeout);
                    doomscrollScrollTimeout = null;
                }
                // Cancel any pending close animation from a previous closeDoomscrollComments call
                if (doomscrollCloseTimeout) {
                    clearTimeout(doomscrollCloseTimeout);
                    doomscrollCloseTimeout = null;
                }
                ...
                drawer.classList.add('border-t', 'border-slate-200', 'dark:border-neutral-800');
                
                const feed = document.getElementById('doomscroll-feed');
                if (feed) {
                    feed.classList.add('shift-up');
        ...
        window.closeDoomscrollComments = function() {
            const drawer = document.getElementById('doomscroll-comments-drawer');
            if (drawer) {
                ...
                const feed = document.getElementById('doomscroll-feed');
                if (feed) {
                    feed.classList.remove('shift-up');
                    feed.style.overflowY = 'scroll';
                }
                if (doomscrollCloseTimeout) clearTimeout(doomscrollCloseTimeout);
                doomscrollCloseTimeout = setTimeout(() => {
                    doomscrollCloseTimeout = null;
                    drawer.classList.add('translate-y-full', 'pointer-events-none');
                    drawer.style.transform = '';
                    drawer.style.display = 'none';
                    drawer.classList.remove('border-t', 'border-slate-200', 'dark:border-neutral-800');
                }, 300);
            }
        };
        
        window.makeCommentsDrawerDraggable = function() {
            const drawer = document.getElementById('doomscroll-comments-drawer');
            if (!drawer) return;
            
            // Prevent clicks/touches inside the drawer from bubbling to feed handlers
            ...
            const notch = document.getElementById('doomscroll-comments-notch') || drawer.firstElementChild;
            if (!notch) return;
            ...
            const handleMove = (clientY) => {
                ...
                // Allow dragging down (positive) or slightly up (negative)
                const translateY = Math.max(-120, deltaY);
            ...
            const handleEnd = () => {
                if (!isDragging) return;
                isDragging = false;
                notch.style.cursor = 'ns-resize';
                drawer.style.transition = 'transform 0.25s ease-out';
                
                const deltaY = currentY - startY;
                const threshold = Math.min(100, drawer.clientHeight * 0.2);
        ...
        window.toggleDoomscrollCommentLike = function(btn) {
            haptic();
            const container = btn.closest('.flex-col') || btn.closest('.flex');
            const likeCount = container.querySelector('.like-count');
            const svg = btn.querySelector('svg');
            const base = parseInt(likeCount?.dataset?.base) || 0;
            const likeKey = btn.dataset.likeKey;
            
            if (btn.classList.contains('text-red-500')) {
                // Retract like: make hollow gray outline
                ...
                if (likeCount) {
                    likeCount.textContent = base;
                }
                if (likeKey) delete doomscrollTempLikes[likeKey];
            } else {
                // Like comment: make solid filled red
                ...
                if (likeCount) {
                    likeCount.textContent = base + 1;
                }
                if (likeKey) doomscrollTempLikes[likeKey] = true;
            }
        };

        window.stopAllDoomscrollVideos = function() {
            if (doomscrollScrollTimeout) {
                clearTimeout(doomscrollScrollTimeout);
                doomscrollScrollTimeout = null;
            }
            ...
            // Reset feed translation and overflow scroll states on exit
            const feed = document.getElementById('doomscroll-feed');
            if (feed) {
                feed.classList.remove('shift-up');
                feed.style.overflowY = 'scroll';
